...
def render_line_tab(stations: pd.DataFrame, services: pd.DataFrame) -> None:
    ...
    line_options = services[["transport_type", "line"]].drop_duplicates()
    ...
    line_stations = get_stations_for_line(
        stations, services, transport_type, line
    )
    st.metric("Stations found", len(line_stations))
    st.caption(
        "Stations are listed alphabetically because the simplified dataset does "
        "not contain a single authoritative stop order for every trip pattern."
    )

    map_column, list_column = st.columns([3, 2], gap="large")
    with map_column:
        st.pydeck_chart(
            build_station_map(line_stations),
            key="line-stations-map",
            height=520,
        )
    with list_column:
        st.dataframe(
            line_stations[["station_name"]].rename(
...
def main() -> None:
    ...
    with line_tab:
        render_line_tab(stations, services)
    with direct_tab:
